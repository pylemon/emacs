...
;; the struct to save perspective settings
(cl-defstruct (perspective
               (:conc-name persp-)  ;; another short NAME
               (:constructor make-persp-internal))  ;; make-NAME
  name
  buffers
  killed
  local-variables  ;; todo: ?
  (buffer-history buffer-name-history)  ;; todo: ?
  ;; current window configuration of FRAME
...
See also `persp-add-buffer'."
  (persp-protect
    ;; get the first arg of switch-to-buffer
